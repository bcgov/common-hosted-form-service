...
describe("Form Designer", () => {
  ...
  it("Submission revise status Assignment", () => {
    ...
    cy.location("search").then((search) => {
      ...
      cy.get(
        "form > .v-input > .v-input__control > .v-field > .v-field__field > .v-field__input"
      ).click();
      ...
      cy.wait(2000);
      cy.get(
        '[data-test="showStatusList"] > .v-input__control > .v-field > .v-field__append-inner > .mdi-menu-down'
      ).click();
      cy.contains("REVISING").click();
      //cy.get('.v-selection-control > .v-label').click();
      cy.get(".v-chip__content")
        .contains("chefs.testing@gov.bc.ca")
        .should("be.visible");
      ...
      cy.contains(
        "No results found. Please add team members in the draft/submission manage page."
      ).should("be.visible");
      cy.get(
        '[data-test="showRecipientEmail"] > .v-input__control > .v-field > .v-field__append-inner > .mdi-menu-down'
      ).click();
      cy.get("label").contains("Notify all submitters").should("be.visible");
      cy.get(
        '[data-test="canAttachCommentToEmail"] > .v-input__control > .v-selection-control > .v-label'
      ).click();
      cy.get('textarea[rows="1"]').type("some comments");
      cy.get("button").contains("REVISE").click();
      cy.get(
        ":nth-child(1) > .v-checkbox > .v-input__control > .v-selection-control > .v-label"
      ).click();
      cy.wait(2000);
