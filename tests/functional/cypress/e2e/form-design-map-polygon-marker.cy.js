...
describe("Form Designer", () => {
  ...
  it("Checks Map component for Polygon marker", () => {
    ...
    cy.get("div.formio-builder-form").then(($el) => {
      ...
      cy.get('textarea[name="data[description]"]').should(
        "have.attr",
        "placeholder",
        "This will appear below the map"
      );
      cy.get('textarea[name="data[description]"]').type("Map location above");
      cy.wait(1000);
      cy.get('textarea[placeholder="Add a tooltip beside the label"]').type(
      ...
      cy.get(
        'div[class="leaflet-draw-tooltip leaflet-draw-tooltip-single"]'
      ).click();
      cy.get(
        'div[class="leaflet-draw-tooltip leaflet-draw-tooltip-single"]'
      ).click();
      ...
      cy.get(
        'div[class="leaflet-marker-icon leaflet-mouse-marker leaflet-zoom-hide leaflet-interactive"]'
      ).click({ force: true });
  ...
  it("Checks user location feature for a Map component", () => {
    cy.viewport(1000, 1100);
    cy.waitForLoad();
    cy.get('div[title="Click to center the map on your location"]').should(
      "exist"
    );
    ...
    cy.get('img[class="leaflet-tile leaflet-tile-loaded"]')
      .should("have.attr", "src")
      .and("include", "https://c.tile.openstreetmap.org");
  ...
  it("Validate custom base map layers for  Map component", () => {
    ...
    cy.get('input[name="data[availableBaseLayersCustom][0][label]"]').type(
      "test"
    );
    ...
    cy.get('input[name="data[availableBaseLayersCustom][0][url]"]').type(
      customUrl,
      { parseSpecialCharSequences: false }
    );
    cy.get(
      'input[name="data[availableBaseLayersCustom][0][attribution]"]'
    ).click();
    cy.get(
      'input[name="data[availableBaseLayersCustom][0][attribution]"]'
    ).type(
  ...
  it("Checks base map layers for a Map component", () => {
    ...
    cy.contains("span", " Light")
      .prev('input[type="radio"]')
      .should("not.have.attr", "checked");
    cy.contains("span", " Dark")
      .prev('input[type="radio"]')
      .should("not.have.attr", "checked");
    cy.contains("span", " Topographic")
      .prev('input[type="radio"]')
      .should("not.have.attr", "checked");
    cy.wait(2000);
    cy.get('a[title="Draw a marker"]').then(($el) => {
    ...
    cy.get('a[class="leaflet-control-layers-toggle"').then(($el) => {
      ...
      cy.get('input[class="leaflet-control-layers-selector"]').then(($el) => {
        ...
        cy.get(base_select_dark)
          .closest("span")
          .contains(" Dark")
          .click({ force: true });
        cy.get(base_select_dark).check();
        cy.wait(2000);
        cy.get('img[src*="https://b.basemaps.cartocdn.com/dark_all/"]').should(
          "be.visible"
        );
        cy.get(base_select_light)
          .closest("span")
          .contains(" Light")
          .click({ force: true });
        cy.wait(1000);
        ...
        cy.wait(2000);
        cy.get('img[src*="https://b.basemaps.cartocdn.com/light_all/"]').should(
          "be.visible"
        );
        cy.get(base_map_btn).trigger("mouseover", { force: true });
        cy.get(base_select_topographic)
          .closest("span")
        ...
        cy.wait(3000);
        cy.get('img[src*="https://b.tile.opentopomap.org/"]').should(
          "be.visible"
        );
        cy.wait(2000);
        cy.get(base_select_test_custom).check();
        cy.wait(2000);
        cy.get(base_select_test_custom)
          .closest("span")
          .contains("test")
          .should("exist");
      });
    ...
    cy.location("search").then((search) => {
      ...
      cy.get('a[class="leaflet-control-layers-toggle"').then(($el) => {
        ...
        cy.get('input[class="leaflet-control-layers-selector"]').then(($el) => {
          ...
          cy.wait(2000);
          cy.get(
            'img[src*="https://b.basemaps.cartocdn.com/dark_all/"]'
          ).should("be.visible");
          cy.get(base_select_light)
            .closest("span")
            .contains(" Light")
          ...
          cy.wait(2000);
          cy.get(
            'img[src*="https://b.basemaps.cartocdn.com/light_all/"]'
          ).should("be.visible");
          cy.get(base_map_btn).trigger("mouseover", { force: true });
          cy.get(base_select_topographic)
            .closest("span")
      ...
      cy.get('div[class="leaflet-popup-content"]')
        .find("p")
        .contains("Only 3 features per submission")
        .should("be.visible");

      //Validate the feature of deleting existing markers not possible
      cy.get(".leaflet-draw-edit-remove").click();
      cy.get("g")
        .find('path[stroke-linejoin="round"]')
        .then(($el) => {
          const del_polygon = $el[0];
          cy.get(del_polygon).click({ force: true });
        });
      cy.get(
        ":nth-child(2) > .leaflet-draw-actions > :nth-child(1) > a"
      ).click();
      //verify validation message
      cy.get('div[class="leaflet-popup-content"]')
        .find("p")
        .contains("Please do not delete pre-existing features")
        .should("be.visible");
