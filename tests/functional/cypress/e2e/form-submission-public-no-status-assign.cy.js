...
describe("Form Designer", () => {
  ...
  it("Verify public form submission", () => {
    ...
    cy.readFile("cypress/fixtures/formId.json").then(({ formId }) => {
      ...
      cy.waitForLoad();
      cy.get(
        ":nth-child(3) > .v-card > .v-card-text > :nth-child(2) > .v-input__control > .v-selection-control > .v-label > span"
      ).click(); //uncheck for not able to update the status of the form
      cy.get('[data-test="canUpdateStatusOfFormCheckbox"]').should(
        "not.be.checked"
      );
      ...
      cy.get("#logoutButton > .v-btn__content > span").should("not.exist");
      cy.get('[data-test="base-auth-btn"] > .v-btn > .v-btn__content > span', {
        timeout: 15000,
      })
        .should("be.visible")
        .click();
