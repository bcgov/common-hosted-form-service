...
describe("Form Designer", () => {
  ...
  it('"Verify export submission', () => {
    ...
    cy.readFile("cypress/fixtures/formId.json").then(({ formId }) => {
      ...
      cy.get(
        ":nth-child(2) > .v-col > .v-input > .v-input__control > .v-selection-control-group > :nth-child(2) > .v-label > .radioboxLabelStyle"
      ).click();
      cy.get(
      ...
      cy.get("span").contains(
        "1 - Multiple rows per submission with indentation spaces"
      );
  ...
  it("Verify print template functionality", () => {
    ...
    cy.get(".v-slide-group-item--active > .v-btn__content").contains(
      "Browser Print"
    );
    ...
    cy.get(".v-window-item--active > .flex-container > .more-info-link").should(
      "exist"
    );
    ...
    cy.waitForLoad();
    cy.get(
      ".v-selection-control-group > .v-text-field > .v-input__control > .v-field"
    ).click();
    cy.contains("pdf").should("be.visible");
    cy.contains("pdf").click();
    cy.get("#file-input-submit").should("be.enabled");
    cy.get(".v-card-actions > .flex-container > .text-textLink").should(
      "be.enabled"
    );
    ...
    cy.get(".mb-5 > .v-btn__content > span")
      .contains("Export")
      .should("be.visible");
    //Verify export button is enabled
    cy.get(".mb-5").should("be.enabled");
