  ...
  it("Verify draft submission", () => {
    ...
    cy.location("search").then((search) => {
      ...
      cy.readFile("cypress/fixtures/formId.json").then(({ formId }) => {
        ...
        cy.get(
          ".mt-6 > :nth-child(1) > .v-btn > .v-btn__content > span"
        ).click();
      ...
      cy.get(".v-alert__content")
        .contains("Wrong json file format")
        .should("be.visible");
      cy.get("h1").contains("Select JSON file to upload").click();
      ...
      cy.get(".v-alert__content")
        .contains("Your multiple draft upload has been successful!")
        .should("be.visible");
      cy.wait(1000);
      //Verify download template link
      cy.get(
        'i[class="mdi-download mdi v-icon notranslate v-theme--chefsTheme v-icon--size-default mr-1"]'
      ).should("exist");
      //Verify Draft upload
      cy.contains(
        "p",
      ...
      cy.get('input[name="data[simpletextfield]"]')
