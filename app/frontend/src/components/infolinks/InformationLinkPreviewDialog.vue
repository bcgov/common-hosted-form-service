<template>
  <v-row justify="center" class="mb-5">
    <v-dialog v-model="dialog" :width="fcProactiveHelpImageUrl ? '70%' : '40%'" @click:outside="onCloseDialog">
      <v-card>
        <v-container class="overflow-auto">
          <v-row>
            <v-col class="d-flex justify-space-between headerWrapper pa-4">
              ...
              <div class="align-self-center cursor">
                <font-awesome-icon icon="fa-solid fa-xmark" :size="'1x'" inverse @click="onCloseDialog" />
              </div>
            </v-col>
          </v-row>
          <v-row class="mt-6" v-if="fcProactiveHelpImageUrl">
            ...
            <v-col md="6">
              <v-img data-cy="preview_image_field" :src="fcProactiveHelpImageUrl"></v-img>
            </v-col>
          </v-row>
          ...
          <v-row v-if="component && component.isLinkEnabled">
            <v-col class="d-flex flex-row align-center text-decoration-underline linkWrapper">
              <a
                ...
                :target="'_blank'"
                :class="{ disabledLink: component && component.moreHelpInfoLink === '' }"
              >
                <div class="mr-1 cursor">Learn more <font-awesome-icon icon="fa-solid fa-square-arrow-up-right" /></div
              ></a>
            </v-col>
          </v-row>
        </v-container>
