...
describe('FloatButton.vue', () => {
  ...
  it('test that publish button was click', async () => {
    const mockRoute = {
      name: 'FormManage',
      query: {
        d: '0014dfe4-321f-4bc1-9280-e7a1fdeb5dc6',
        f: '01fa4a32-ff4a-4304-8277-e69e0bb2d229',
        fd: 'formDesigner',
      },
    };
    ...
    expect(
      wrapper.findComponent({ ref: 'publishRouterLink' }).props().to
    ).toStrictEqual(mockRoute);
  });
  ...
  it('test that manage button was click', async () => {
    ...
    expect(
      wrapper.findComponent({ ref: 'settingsRouterLink' }).props().to
    ).toStrictEqual(mockRoute);
  });
