...
(function () {
  ...
  class CHEFSFormViewer extends HTMLElement {
    ...
    attributeChangedCallback(name, oldValue, newValue) {
      ...
      if (oldValue !== newValue) {
        switch (name) {
          ...
          case 'theme':
            this.theme = newValue;
            // Re-render to apply new theme styles
            this.render();
            break;
          case 'language':
        ...
        // Debounce loadForm calls to prevent multiple simultaneous loads
        if (this.formId && this.apiKey) {
          ...
          this.loadFormTimeout = setTimeout(() => {
            if (this.formId && this.apiKey && !this.isLoading) {
              console.log('CHEFS Web Component: Debounced loadForm call');
              this.loadForm();
            }
          }, 100);
        }
    ...
    connectedCallback() {
      this.render();
      if (this.formId && this.apiKey) {
        // If baseUrl is not set yet, wait a bit for it to be set
        if (!this.baseUrl) {
          setTimeout(() => {
            if (this.formId && this.apiKey) {
              this.loadForm();
    ...
    async getFormSchema() {
      ...
      const response = await fetch(
        `${baseUrl}/api/v1/webcomponent/${this.formId}/schema`,
        {
          headers: {
    ...
    async handleSubmit(submission) {
      ...
      try {
        ...
        console.log(
          ...
          `${baseUrl}/api/v1/webcomponent/${this.formId}/submit`
        );
