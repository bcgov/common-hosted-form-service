...
describe('earch BCGEO Address', () => {
  it('should call searchBCGeoAddress method ', async () => {
    let res = jest.fn();
    let next = jest.fn();
    service.searchBCGeoAddress = jest.fn().mockReturnValue({'type':'FeatureCollection','queryAddress':25,'features':[{'type':'Feature','geometry':{'type':'Point','crs':{'type':'EPSG','properties':{'code':4326}},'coordinates':[-118.4683663,49.0257091]},'properties':{'fullAddress':'25th St, Grand Forks, BC','score':61,'matchPrecision':'STREET','siteStatus':''}}]});
