...
exports.up = function (knex) {
  return Promise.resolve()
    ...
    .then(() =>
      knex.schema.createTable('form_identity_provider', (table) => {
        table.uuid('id').primary();
    ...
    .then(() =>
      knex.schema.createTable('form_role_user', (table) => {
        table.uuid('id').primary();
    ...
    .then(() =>
      knex.schema.createTable('form_version', (table) => {
        table.uuid('id').primary();
        table.uuid('formId').references('id').inTable('form').notNullable().index();
    ...
    .then(() =>
      knex.schema.createTable('form_submission', (table) => {
        table.uuid('id').primary();
