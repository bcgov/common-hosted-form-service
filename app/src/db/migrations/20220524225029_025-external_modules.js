  ...
  return Promise.resolve()
    ...
    .then(() =>
      ...
      knex.schema.createTable('form_module_version', table => {
        table.uuid('id').primary();
        table.uuid('formModuleId').notNullable().index()
          .references('id').inTable('form_module')
          .onUpdate('CASCADE')
          .onDelete('CASCADE');
    ...
    .then(() =>
      ...
      knex.schema.createTable('form_module_identity_provider', table => {
        ...
        table.string('code').notNullable()
          .references('code').inTable('identity_provider');
        stamps(knex, table);
      }))
    .then(() =>
      ...
      knex.schema.createTable('form_version_form_module_version', table => {
        table.uuid('id').primary();
        table.uuid('formVersionId').notNullable().index()
          .references('id').inTable('form_version')
          .onUpdate('CASCADE')
          .onDelete('CASCADE');
        table.uuid('formModuleVersionId').notNullable().index()
          .references('id').inTable('form_module_version')
          .onUpdate('CASCADE')
          .onDelete('CASCADE');
        stamps(knex, table);
      }));
