...
routes.get('/:fileId', apiAccess.checkApiKey, basicApiAccess, currentFileRecord, hasFilePermissions([P.SUBMISSION_READ]), async (req, res, next) => {
  await controller.read(req, res, next);
});

routes.get('/:fileId/clone', apiAccess.checkApiKey, basicApiAccess, currentFileRecord, hasFilePermissions([P.SUBMISSION_READ]), async (req, res, next) => {
  await controller.clone(req, res, next);
});

routes.delete('/:fileId', apiAccess.checkApiKey, basicApiAccess, currentFileRecord, hasFilePermissions([P.SUBMISSION_UPDATE]), async (req, res, next) => {
