...
const service = {
  ...
  uploadPackage: async ({ filename, contentType, buffer }, folder = 'uploads') => {
    // Stage inside the upload dir (not bare /tmp): objectStorageService reads the
    // temp file back through a containment check scoped to getFileUploadsDir(),
    // so a path outside it makes fileService.create throw (see CCP-5262).
