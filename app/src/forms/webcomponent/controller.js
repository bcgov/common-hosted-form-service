...
module.exports = {
   ...
   */
  readFormSchemaForWidget: async (req, res, next) => {
    try {
   ...
   * @param {Object} next the Express chaining function
   */
  createSubmissionForWidget: async (req, res, next) => {
    try {
      const formId = req.params.formId;
      if (!validate(formId)) {
        res.status(400).json({ detail: `Bad formId "${formId}".` });
      ...
      if (!req.body.draft) {
        emailService.submissionReceived(formId, response.id, req.body, req.headers.referer).catch(() => {});
      }
